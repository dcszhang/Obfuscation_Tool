...
def process_second_feature(ssa):
    ...
    for function in sorted(ssa.functions, key=lambda f: f.offset):
        ...
        pdg = Obfuscation.ProgramDependenceGraph(function)
    ...
    function_embeddings, adjacency_matrices = generate_sdg(ssa_functions, "word2vec_Train/word2vec.model")
    ...
    #         print(f"  Block {hex(block_offset)}: {embedding[:5]}...")  
    ...
    if len(rgcn_data_list) == 0:
        print("No samples found in rgcn_data_list. Skipping DataLoader initialization.")
        return  
    loader = DataLoader(rgcn_data_list, batch_size=len(rgcn_data_list), shuffle=False)
    # for data in loader:
    ...
    # print("(5)Graph Pairwise Similarities:")
    max_similarity = 0
    max_pair = None
    # 遍历所有图对以找到最大相似度
    for i in range(num_graphs):
        for j in range(i + 1, num_graphs):
            similarity = similarity_matrix[i, j].item() * 100
            if similarity > max_similarity:
                max_similarity = similarity
                max_pair = (i, j)
    # 输出最大相似度及其对应的图对
    if max_pair:
        print("(5)The maximum similarity between graphs:")
        print(f"    Graph {max_pair[0]} and Graph {max_pair[1]}: {max_similarity:.4f}%")
        print("-----------------------------------------------------------")
    else:
...
def generate_sdg(ssa_functions, model_path):
    ...
    sdg = Obfuscation.SystemDependenceGraph(ssa_functions)
...
def prepare_rgcn_data(function_embeddings, adjacency_matrices):
    ...
    for func_key in function_embeddings.keys():
        ...
        # )
        values_list = list(function_embeddings[func_key].values())

        # 将列表转换为 numpy.ndarray
        values_array = np.array(values_list)
        # 再将 numpy.ndarray 转换为 PyTorch tensor
        node_features = torch.tensor(values_array, dtype=torch.float)
