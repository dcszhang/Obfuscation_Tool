...
from Obfuscation.evmasm import EVMAsm
...
class StackValue(object):
    ...
    def resolve(self, visited=None) -> Tuple['StackValue', bool]:
        return (self, False)
...
class PlaceholderStackValue(StackValue):
    ...
    def resolve(self, visited=None) -> Tuple[StackValue, bool]:
        if visited is None:
            visited = set()  # 初始化访问记录集合

        # 检查是否存在循环依赖
        if self in visited:
            # print(f"Detected circular dependency at {self}")
            return self, False  # 返回未解析状态以避免死循环
        ...
        if len(self.block.in_edges) == 1:
            ...
            if isinstance(new_slot, PlaceholderStackValue):
                rv = new_slot.resolve(visited)  # 递归解析时传递访问记录
                self.resolving = False
                return rv
        ...
        if len(self.block.in_edges) > 1:
            ...
            for edge in sorted(self.block.in_edges, key=lambda bb: bb.offset):
                ...
                try:
                    ...
                    new_slot, _ = new_slot.resolve(visited)  # 递归解析时传递访问记录
                except IndexError:
                    ''' 
